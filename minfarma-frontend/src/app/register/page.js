...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (res.ok) {
      ...
      alert('Registrasi Gagal, email mungkin sudah dipakai.');
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <form onSubmit={handleRegister} className="bg-white p-8 rounded-lg shadow-md w-96">
        ...
        <input type="text" placeholder="Nama Lengkap" onChange={(e) => setName(e.target.value)} className="w-full mb-4 p-2 border rounded focus:outline-none focus:ring-2 focus:ring-red-700 text-black" required/>
        ...
        <p className="text-center text-sm text-gray-600">
          Sudah punya akun? <Link href="/" className="text-red-700 font-bold hover:underline">Login di sini</Link>
        </p>
      </form>
    </div>
  );
